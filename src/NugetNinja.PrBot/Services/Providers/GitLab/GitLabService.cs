...
public class GitLabService : IVersionControlService
{
    ...
    public async IAsyncEnumerable<Repository> GetMyStars(string endPoint, string userName, string patToken)
    {
        ...
        for (var i = 1; ; i++)
        {
            var endpoint = $@"{endPoint}/users/{userName}/starred_projects?page={i}";
    ...
    {
        _logger.LogInformation($"Forking repository on GitLab with org: {org}, repo: {repo}...");
    ...
    public async Task<List<PullRequest>> GetPullRequests(string endPoint, string org, string repo, string head,
        string patToken)
    {
        _logger.LogInformation($"Getting pull requests on GitLab with org: {org}, repo: {repo}...");
    ...
    public async Task CreatePullRequest(string endPoint, string org, string repo, string head, string @base,
        string patToken)
    {
        _logger.LogInformation($"Creating a new pull request on GitLab with org: {org}, repo: {repo}...");
