...
public class GitLabService(HttpWrapper httpClient, ILogger<GitLabService> logger) : IVersionControlService
{
    ...
    public async Task<IReadOnlyCollection<Aiursoft.NugetNinja.GitServerBase.Models.Abstractions.MergeRequestSearchResult>> GetOpenMergeRequests(string endPoint, string userName, string patToken)
    {
        ...
        var mergeRequests = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Aiursoft.NugetNinja.GitServerBase.Models.Abstractions.MergeRequestSearchResult>>(json);
        return mergeRequests!;
    }

    public async Task<Aiursoft.NugetNinja.GitServerBase.Models.Abstractions.DetailedMergeRequest> GetMergeRequestDetails(string endPoint, string userName, string patToken, int projectId, int mergeRequestId)
    {
        ...
        var json = await httpClient.SendHttp(endpoint, HttpMethod.Get, patToken);
        var mergeRequest = Newtonsoft.Json.JsonConvert.DeserializeObject<Aiursoft.NugetNinja.GitServerBase.Models.Abstractions.DetailedMergeRequest>(json);
        return mergeRequest!;
    }
